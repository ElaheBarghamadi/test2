...
import { ArrowUpDown, CheckCircle2, ClipboardList, Download, Search, Send } from "lucide-react";
import { useRouter } from "next/navigation";
import { useEffect, useMemo, useState } from "react";
import { examsApi } from "@/lib/api/exams";
import { resultsApi } from "@/lib/api/results";
import { RESULT_DETAIL_OPTIONS, resolveResultDetail, type ResultDetailChoice } from "@/lib/exam/result-detail";
...
export default function TeacherResultsPage() {
  ...
  const [rows, setRows] = useState<ApiTeacherResultRowDto[]>([]); const [rowsLoading, setRowsLoading] = useState(false); const [error, setError] = useState<string | null>(null);
  ...
  /**
   * Marking lives on its own desk, not in a dialog over this table: a paper needs the whole sheet in front
   * of the teacher, and the question-by-question pass needs the width for the whole cohort at once.
   */
  function openMarking(attemptId?: string, mode: "sheet" | "question" = "sheet") {
    const params = new URLSearchParams();
    if (mode === "question") params.set("mode", "question");
    if (attemptId) params.set("attempt", attemptId);
    const search = params.toString();
    router.push(`/teacher/exams/${examId}/marking${search ? `?${search}` : ""}`);
  }
  const [publishOpen, setPublishOpen] = useState(false);
  const [publishDetail, setPublishDetail] = useState<ResultDetailChoice>("score_only");
  const [publishBusy, setPublishBusy] = useState(false);
  /** Publishing is where the choice belongs: it is the moment the teacher decides what students may read. */
  function openPublish() { if (!exam) return; setPublishDetail(resolveResultDetail(exam.settings)); setPublishOpen(true); }
  async function publish() {
    if (!exam) return;
    setPublishBusy(true);
    try {
      // Only a changed rung is written, so publishing what is already chosen never re-saves the exam.
      if (publishDetail !== resolveResultDetail(exam.settings)) await examsApi.updateSettings(exam.id, { result_detail: publishDetail });
      const result = await resultsApi.publishExamResults(exam.id);
      await loadRows();
      setPublishOpen(false);
      const rung = RESULT_DETAIL_OPTIONS.find((option) => option.value === publishDetail)?.short ?? "";
      toast({ title: `${toPersianNumber(result.published_count)} نتیجه منتشر شد`, description: result.pending_manual_grading_count ? `${toPersianNumber(result.pending_manual_grading_count)} پاسخ هنوز نیازمند تصحیح است.` : `در سطح «${rung}» برای دانش‌آموزان قابل مشاهده شد.`, variant: "success" });
    } catch (reason) { toast({ title: "انتشار نتیجه انجام نشد", description: apiErrorMessage(reason), variant: "error" }); }
  ...
  return <PageTransition><PageContainer><PageHeader eyebrow="تحلیل عملکرد" title="نتایج آزمون‌ها" description="تلاش‌های واقعی دانش‌آموزان را بررسی، پاسخ‌های متنی را تصحیح و نتیجه‌ها را منتشر کنید." breadcrumbs={[{ label: "آموزگار", href: "/teacher/dashboard" }, { label: "نتایج" }]} action={<Button variant="outline" onClick={downloadReport} disabled={!rows.length}><Download className="h-4 w-4"/>دریافت گزارش</Button>}/>{loading && !initialized ? <ResultsSkeleton/> : !available.length ? <EmptyState title="آزمون قابل گزارش ندارید" description="پس از انتشار یا شروع نخستین آزمون، تلاش‌های دانش‌آموزان اینجا نمایش داده می‌شوند."/> : <><Card><CardContent className="p-5 sm:p-6"><label className="block max-w-md text-xs font-bold text-muted-foreground">انتخاب آزمون<select value={exam?.id ?? ""} onChange={(event) => setExamId(event.target.value)} className="mt-2 h-11 w-full rounded-xl border bg-background px-3 text-sm font-extrabold text-foreground">{available.map((item) => <option key={item.id} value={item.id}>{item.title}</option>)}</select></label><div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4"><div className="flex items-center gap-4 rounded-2xl bg-muted/45 p-4"><DonutChart value={Math.round((average / Math.max(exam?.settings.totalMarks || 1, 1)) * 100)}/><div><p className="text-xs font-bold text-muted-foreground">میانگین تلاش‌های نهایی</p><p className="mt-1 text-lg font-black">{toPersianNumber(average.toFixed(1))} <span className="text-xs text-muted-foreground">از {toPersianNumber(exam?.settings.totalMarks ?? 0)}</span></p></div></div><ResultMetric label="نرخ تکمیل" value={`${toPersianNumber(Math.round((finalized.length / Math.max(rows.length, 1)) * 100))}٪`} detail={`${toPersianNumber(finalized.length)} تلاش نهایی از ${toPersianNumber(rows.length)}`} tone="text-primary"/><ResultMetric label="نیازمند تصحیح" value={toPersianNumber(needsGrading)} detail="پاسخ متنی در صف بررسی" tone="text-amber-600"/><ResultMetric label={passMark > 0 ? "نرخ قبولی" : "حدنصاب قبولی"} value={passMark > 0 ? `${toPersianNumber(Math.round((passedCount / Math.max(graded.length, 1)) * 100))}٪` : "تعیین‌نشده"} detail={passMark > 0 ? `${toPersianNumber(passedCount)} از ${toPersianNumber(graded.length)} تلاشِ نمره‌گرفته` : "برای محاسبهٔ حکم، در تنظیمات آزمون حداقل نمره بگذارید"} tone={passMark > 0 ? "text-emerald-600" : "text-muted-foreground"}/></div><div className="mt-5 flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={() => openMarking(undefined, "question")} disabled={!finalized.length}><ClipboardList className="h-4 w-4"/>تصحیح سؤال‌به‌سؤال</Button><Button onClick={openPublish} disabled={!rows.length}><Send className="h-4 w-4"/>انتشار نتایج آماده</Button></div></CardContent></Card>{queue.length > 0 && <Card className="mt-6"><CardHeader className="gap-2 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle>صف تصحیح</CardTitle><CardDescription>{toPersianNumber(queue.length)} تلاش با پاسخ‌های نمره‌نگرفته؛ کم‌پیشرفته‌ترین‌ها اول.</CardDescription></div>{queueLoading && <span className="text-[11px] text-muted-foreground">در حال بارگیری…</span>}</CardHeader><CardContent><div className="flex gap-2 overflow-x-auto pb-1">{queue.map((item) => { const done = item.graded_count; const total = item.manual_count; return <button key={item.attempt_id} type="button" onClick={() => { setExamId(item.exam_id); router.push(`/teacher/exams/${item.exam_id}/marking?attempt=${item.attempt_id}`); }} className="min-w-56 shrink-0 rounded-2xl border bg-card p-3 text-right transition-colors hover:border-primary/45"><p className="truncate text-xs font-black">{item.student_name}</p><p className="mt-0.5 truncate text-[10px] text-muted-foreground">{item.exam_title} · تلاش {toPersianNumber(item.attempt_number)}</p><p className="mt-2 text-[11px] font-bold">{toPersianNumber(done)} / {toPersianNumber(total)} تصحیح‌شده</p><div className="mt-1 h-1.5 overflow-hidden rounded-full bg-muted" role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuenow={item.progress} aria-label={`پیشرفت تصحیح ${item.student_name}`}><div className="h-full rounded-full bg-amber-500" style={{ width: `${item.progress}%` }}/></div></button>; })}</div></CardContent></Card>}
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-end gap-2 border-t pt-4">
            <Button variant="outline" size="sm" onClick={() => setPublishOpen(false)}>انصراف</Button>
            <Button size="sm" onClick={() => void publish()} disabled={publishBusy}><Send className="h-4 w-4"/>{publishBusy ? "در حال انتشار" : "انتشار"}</Button>
          </div>
        </div>
      </Dialog></PageContainer></PageTransition>;
}
