...
import type { ApiTeacherAttemptDetailDto, ApiTeacherResultRowDto } from "@/lib/api/dtos";
...
export default function TeacherResultsPage() {
  ...
  async function loadRows(id = examId) { if (!id) return; setRowsLoading(true); setError(null); try { setRows(await resultsApi.teacherExamRows(id)); } catch (reason) { setError(apiErrorMessage(reason, "دریافت نتایج آزمون انجام نشد.")); } finally { setRowsLoading(false); } }
  ...
  const finalized = rows.filter((row) => row.submission_status !== "in_progress"); const average = finalized.length ? finalized.reduce((sum, row) => sum + numeric(row.score), 0) / finalized.length : 0; const needsGrading = rows.filter((row) => row.submission_status === "needs_grading").length;
...
function GradeDialog({ attempt, onClose, onUpdated, afterSave }: { attempt: ApiTeacherAttemptDetailDto | null; onClose: () => void; onUpdated: (attempt: ApiTeacherAttemptDetailDto) => void; afterSave: () => void }) {
  ...
  return <Dialog open={Boolean(attempt)} onClose={onClose} title={attempt ? `پاسخ‌های ${attempt.student.full_name}` : "پاسخ‌ها"} description={attempt ? `${attempt.exam.title} · ${attempt.student.grade} ${attempt.student.class_name}` : undefined} size="md">
    {manual.length ? <div className="max-h-[52vh] space-y-4 overflow-y-auto pl-1">{manual.map((answer) => <div className="rounded-2xl border p-4" key={answer.id}><div className="flex items-start justify-between gap-4"><p className="text-sm font-black">{toPersianNumber(answer.question_order)}. {answer.question_text}</p><Badge variant={answer.manual_score === null ? "warning" : "success"}>{toPersianNumber(numeric(answer.maximum_score))} نمره</Badge></div><p className="mt-3 rounded-xl bg-muted/60 p-3 text-sm leading-7 text-muted-foreground">{answer.text || "دانش‌آموز پاسخی ثبت نکرده است."}</p><div className="mt-3 grid gap-3 sm:grid-cols-[130px_1fr_auto]"><Input type="number" min="0" max={numeric(answer.maximum_score)} step="0.25" value={scores[answer.question_id] ?? ""} onChange={(event) => setScores((old) => ({ ...old, [answer.question_id]: event.target.value }))} placeholder={`از ${numeric(answer.maximum_score)}`}/><Textarea value={feedback[answer.question_id] ?? ""} onChange={(event) => setFeedback((old) => ({ ...old, [answer.question_id]: event.target.value }))} className="min-h-11" placeholder="بازخورد این پاسخ…"/><Button size="sm" onClick={() => void grade(answer.question_id)} disabled={savingId === answer.question_id}>{savingId === answer.question_id ? "ذخیره…" : "ثبت نمره"}<Star className="h-3.5 w-3.5"/></Button></div></div>)}</div> : <p className="rounded-xl bg-muted p-4 text-sm text-muted-foreground">پاسخ متنی برای تصحیح دستی در این تلاش ثبت نشده است.</p>}
