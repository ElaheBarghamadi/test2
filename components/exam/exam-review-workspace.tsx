...
export function ExamReviewWorkspace({ exam, remote = false }: { exam: Exam; remote?: boolean }) {
  ...
  async function submit() {
    ...
    try {
      // Flush the final locally queued answer/flag mutations before finalization. The same
      // PATCH endpoints are idempotent, so an overlapping autosave is harmless.
      if (attempt.pendingAnswerQuestionIds?.length || attempt.pendingFlagQuestionIds?.length) {
        await examAttemptService.saveAnswers({ attempt, exam, revision: attempt.answerRevision });
        markSaved(attempt.answerRevision);
      }
      beginSubmission();
      await examAttemptService.submitAttempt(attempt);
      finishSubmission();
      toast({ title: "آزمون با موفقیت ارسال شد", description: "وضعیت نتیجه برای شما آماده است.", variant: "success" });
