...
export function Topbar({ role, user }: { role: Role; user: User }) {
 ...
 return <><header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b bg-background/85 px-4 backdrop-blur-md sm:px-6 lg:px-8">
   ...
   <div className="mr-auto flex items-center gap-1.5"><NotificationBell/><ThemeToggle/><div className="relative"><button onClick={() => setMenu(!menu)} aria-expanded={menu} className="mr-1 flex items-center gap-2 rounded-xl p-1.5 hover:bg-muted"><div className="hidden text-right sm:block"><p className="max-w-36 truncate text-xs font-extrabold">{user.fullName}</p><p className="max-w-36 truncate text-[10px] text-muted-foreground">{user.schoolName || roleLabel[role]}</p></div><Avatar name={user.fullName} size="sm"/><ChevronDown className="h-3 w-3 text-muted-foreground"/></button>{menu && <div className="absolute left-0 top-12 w-44 rounded-xl border bg-card p-1.5 shadow-lift"><button className="w-full rounded-lg px-3 py-2 text-right text-xs font-bold hover:bg-muted" onClick={() => { setMenu(false); router.push(profilePathForRole(role)); }}>پروفایل و امنیت</button><button className="w-full rounded-lg px-3 py-2 text-right text-xs font-bold hover:bg-muted" onClick={() => { setMenu(false); void logout().finally(() => router.replace("/login")); }}>خروج</button></div>}</div></div>
