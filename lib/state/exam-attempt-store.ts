...
export const useExamAttemptStore = create<AttemptState>((set, get) => ({
  ...
  hydrateRemote: (attempt) => set({ attempt: { ...attempt, pendingAnswerQuestionIds: [], pendingFlagQuestionIds: [] } }),
  ...
  markSaved: (revision) => set((state) => {
    ...
    return { attempt: { ...state.attempt, pendingAnswerQuestionIds: [], pendingFlagQuestionIds: [], saveStatus: "saved", lastSavedAt: new Date().toISOString() } };
  }),
